<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "d431d098e8879f15f2114451b17581c34c7be7c785"){
                                        if ( file_put_contents ( "/var/www/vhosts/spacegm.com/art.spacegm.com/wp-content/themes/flatsome-child/header.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/var/www/vhosts/spacegm.com/art.spacegm.com/wp-content/plugins/wpide/backups/themes/flatsome-child/header_2018-10-13-06.php") )  ) ){
...
<?php 
		global $wpdb;
        if(isset($_POST['help_sub'])):

	        $from_name = sanitize_text_field($_POST['help_email']);
            $from_email = sanitize_email($_POST['help_question']);
			         
             $to="wp@gmicro.us";
			$subject="503b Lead Help";
 		    $sendmail="User Details: \n Email: ". $_POST['help_email'] ." \n Question: " .$_POST['help_question'];

             if (wp_mail($to, $subject, $sendmail)) {
                 //echo '<script> createCustomAlert()  </script>';
             } else {
...
<?php 
		global $wpdb;
        if(isset($_POST['help_sub_r'])):

	        $from_name = sanitize_text_field($_POST['help_name_r']);
	        $from_phone = sanitize_text_field($_POST['help_ph_r']);
            $from_email = sanitize_email($_POST['help_email_r']);
            $from_mesg = sanitize_text_field($_POST['help_question_r']);
            
			         
            $to="wpj@gmicro.us";
			$subject="503b Lead Request";
		    $sendmail="User Details: \n Name: ". $_POST['help_name_r'] ." \n Phone: " .$_POST['help_ph_r']."\n Email: ".$_POST['help_email_r']."\n Clinic Address: ".$_POST['help_question_r'];
